	...
	if($mobileExists && $pluginMobile && $pluginMobile->isActive()) {
		...
		if(file_exists($platform_mobile) && file_exists($platform_homebridge)) {
			if(filemtime($platform_mobile) > filemtime($platform_homebridge)) {
				...
				exec('yes | ' . system::getCmdSudo() . ' cp '.$platform_mobile.' '.$platform_homebridge.' >> ' . log::getPathToLog('homebridge') . ' 2>&1 ');	
			}
		...
		if(file_exists($AccessoryInfoMobile) && !file_exists($AccessoryInfoHomebridge)) {
			...
			exec('yes | ' . system::getCmdSudo() . ' cp '.$AccessoryInfoMobile.' '.$AccessoryInfoHomebridge.' >> ' . log::getPathToLog('homebridge') . ' 2>&1 ');
		}
		...
		if(file_exists($IdentifierCacheMobile) && !file_exists($IdentifierCacheHomebridge)) {
			...
			exec('yes | ' . system::getCmdSudo() . ' cp '.$IdentifierCacheMobile.' '.$IdentifierCacheHomebridge.' >> ' . log::getPathToLog('homebridge') . ' 2>&1 ');
		}
		...
		if(file_exists($cachedAccessoriesMobile) && !file_exists($cachedAccessoriesHomebridge)) {
			...
			exec('yes | ' . system::getCmdSudo() . ' cp '.$cachedAccessoriesMobile.' '.$cachedAccessoriesHomebridge.' >> ' . log::getPathToLog('homebridge') . ' 2>&1 ');
		}
	}
