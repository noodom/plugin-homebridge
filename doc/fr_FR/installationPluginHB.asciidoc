...
*Pour les migrations depuis le plugin mobile officiel, il est important de ne pas désactiver le plugin mobile. Une rubrique "Migration depuis le plugin mobile" est disponible dans la documentation.* 
...
* Tout système basé sur Debian 8

[IMPORTANT]
*Les installations sous Docker et Raspberry Pi 1 ne sont pas compatibles avec cette version de Homebridge.*

Une fois les dépendances installées, le démon se lance. Si le statut n'est pas sur "OK", il faut cliquer sur "(Re)Démarrer".
