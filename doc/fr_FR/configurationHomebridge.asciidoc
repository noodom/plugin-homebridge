...
[IMPORTANT]
Les PIN suivants ne sont pas accéptés par Apple : 000-00-000, 111-11-111, 222-22-222 -> 999-99-999, 123-45-678, 876-54-321. Son changement obligera la reconfiguration de vos applications Homekit.
...
La majorité des types génériques est déjà renseignée. Dans certains cas, une configuration manuelle est necessaire (pour le plugin Virtuel par exemple).
...
Il existe plusieurs applications sur l'appstore compatible homekit. L'application "maison" d'Apple sera utilisée pour la rédaction de la documentation.
...
[TIP]
Dans notre exemple, notre domicile s'appelle "Test". Son nom peut modifier en allant dans les réglages de l'application.
...
Sélectionnez le pont à inclure.

image::../images/home-3.jpg[]


[IMPORTANT]
Comme expliqué plus haut dans la doc, Homebridge n'est pas reconnu officiellement par Apple. Un message indique que l'accessoire n'est pas cetifié il faut valider l'inclusion en cliquant sur "poursuivre l'ajout".
...
Les accessoires doivent être rangés correctement dans homekit. Il faudra créer des pièces et y intégrer les accessoires.
...
Il faut sélectionner la pièce où sera installée le pont. Si elle n'existe pas, il faudra la créer en cliquant sur "Créer".
...
Maintenant, il ne vous reste plus qu'a ranger tous les accessoires. 
