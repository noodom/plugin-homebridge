...
Suivant les configurations matèriels, la qualité de la transmition peut varier. Par exemple, sur un NUC gen7 core i7, c'est quasiment du live ! La latence est très faible.

Celles-ci peuvent afficher une notification lorsque un mouvement est détecté par une capteur de présence. Il faut que la caméra et le capteur soient configurés dans la même pièce et que les notifications du capteurs soient activées.

image::../images/notif.jpg[]

Les caméras décritent ci dessous ont été testées. Elles sont donc fonctionnelles dans homebridge.
