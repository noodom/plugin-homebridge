...
HomeBridge-Präsentation

*Das HomeBridge-Plugin* ist ein Daemon, mit dem Sie über den Siri-Sprachassistenten auf iOS mit einem Heimautomationssystem interagieren können. Das HomeKit wurde seit iOS 8 eingeführt, ist aber erst ab iOS 10 über die Home-Anwendung voll funktionsfähig. 

image::../images/homekit-logo.jpg[]

Das Homebridge-Plugin von Jeedom ermöglicht es, Jeedom-Geräte freizugeben, die mit dem *HomeKit*-Protokoll kompatibel sind.

[IMPORTANT]
*HomeBridge wird von Apple offiziell nicht unterstützt. Apple kann dieses Protokoll daher jederzeit sperren.*

=== Was können Sie mit HomeBridge machen?

HomeBridge kann mit einer HomeKit-kompatiblen Anwendung oder mit dem Siri Sprachassistenten verwendet werden.

Seit iOS 10 ermöglicht die Home-Anwendung (standardmäßig im Lieferumfang von iOS enthalten) die Steuerung von HomeKit-kompatiblen Geräten. 

image::../images/cuisine-homekit.jpg[]

Die Geräte können nach Räumen klassifiziert werden.

image::../images/piece-homekit.jpg[]

Viele Zubehörteile werden unterstützt.

image::../images/garage-homekit.png[]

Siri kann auch interagieren. Sie beantwortet Fragen : 


image::../images/siri-01.jpg[]

Siri kann auch aktiv werden : 

image::../images/siri-02.jpg[]

HomeKit hat den Vorteil, dass es auch außerhalb des Hauses einsetzbar ist. Die einzige Bedingung ist, dass Sie einen Hub benötigen. 
Das iPad und AppleTV (und bald auch der HomePod) können als Hub genutzt werden. Dazu müssen sie mit demselben iCloud-Konto verbunden sein.


[TIPP]
*HomeKit ist der offizielle Name des von Apple entwickelten Protokolls. HomeBridge ist das von nfarina entwickelte Open Source Äquivalent. Letzteres hat das HAP-NodeJS-Projekt, das die treibende Kraft hinter HomeBridge ist, erweitert.*

=== HomeKit-kompatibler Zubehörtyp (wird nach und nach zur HomeBridge hinzugefügt)
