...
class homebridge extends eqLogic {
	...
	public static function generate_file(){
		...
		$user = user::byId(config::byKey('user_homebridge','homebridge',1,true));
		if(is_object($user)){
			$apikey = $user->getHash();
		}else{
			$apikey = config::byKey('api');
		}
		if(in_array(config::byKey('pin_homebridge','homebridge','031-45-154',true),self::DisallowedPIN())) {
			log::add('homebridge', 'error', 'Le PIN Homebridge n\'est pas autorisée par Apple'.config::byKey('pin_homebridge','homebridge','031-45-154',true));	
		}
		
		$HomebridgeUserName = config::byKey('mac_homebridge','homebridge',self::generateRandomMac(),true);
		config::save('mac_homebridge',$HomebridgeUserName,'homebridge');
		
		$response = array();
		$response['bridge'] = array();
		$response['bridge']['name'] = config::byKey('name_homebridge','homebridge',config::byKey('name'),true);
		$response['bridge']['username'] = $HomebridgeUserName;
		$response['bridge']['port'] = 51826;
		$response['bridge']['pin'] = config::byKey('pin_homebridge','homebridge','031-45-154',true);
	...
	public static function deamon_start($_debug = false) {
		...
		log::add('homebridge', 'info', 'Démarrage avahi-daemon : ' . $cmd);
		exec($cmd . ' >> ' . log::getPathToLog('homebridge_homebridge') . ' 2>&1 &');
		...
		log::add('homebridge', 'info', 'Vérification IP Multiples : ' . $cmd);
		exec($cmd . ' >> ' . log::getPathToLog('homebridge_homebridge') . ' 2>&1 &');
	}
	...
	*/
	...
	public static function repairHomebridge($reinstall=true) {
		log::add('homebridge', 'info', 'Procedure de réparration');
		...
		log::add('homebridge', 'info', 'création d\'une nouvelle MAC adress');
		config::save('mac_homebridge',self::generateRandomMac(),'homebridge');
		config::save('name_homebridge',config::byKey('name').'_Repaired_'.base_convert(mt_rand(0,255),10,16),'homebridge');
		if($reinstall) {
			log::add('homebridge', 'info', 'réinstallation des dependances');
			homebridge::dependancy_install();
		}
		else {
			log::add('homebridge', 'info', 'Géneration du fichier de configuration');
